...
function ArticleById() {
  ...
  useEffect(() => {
    ...
    const getArticle = async () => {
      ...
      try {
        const res = await axios.get(`${import.meta.env.VITE_URL}/user-api/articles/${id}`, { withCredentials: true });
  ...
  const addComment = async (commentObj) => {
    ...
    try {
        ...
        if (res.status === 200) {
            setArticle(res.data.payload);  // ← was res.data.comment
            reset({comment: ""}); // ← to clear the input field after successful comment
        }
    } catch(err) {
